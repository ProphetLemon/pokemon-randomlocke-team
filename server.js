...
app.post("/pokemon", async (req, res) => {
  ...
  for (let ability of pokemon.abilities) {
    ...
    abilities += `<li><b>${abilityInfo.names[5].name}${
      ability.is_hidden ? " (hab. oculta)" : ""
    }: </b>`;
  ...
  var imagen = pokemon.sprites.front_default
    ? `<img class="pokemonIcon" src="${pokemon.sprites.front_default}" />`
    : '<img class="pokemonIco" src="/img/missingno.png" />';
  var shiny = pokemon.sprites.front_shiny
    ? `<img class="pokemonIcon" src="${pokemon.sprites.front_shiny}" />`
    : '<img class="pokemonIco" src="/img/missingnoshiny.png" />';
  ...
  res.send(`
        ...
        <tr><td class="border border-secondary">${
          legendario ? "<b>LEGENDARIO</b>" : mitico ? "<b>MÍTICO</b>" : "Común"
        }</td><td class="border border-secondary">Nº ${
    especie.pokedex_numbers[0] ? especie.pokedex_numbers[0].entry_number : ""
  }${
    especie.pokedex_numbers[0]
      ? especie.pokedex_numbers[0].pokedex.name == "national"
        ? ""
        : ` - ${especie.pokedex_numbers[0].pokedex.name}`
      : "?"
  }</td></tr>
        ...
        <tr><td class="border border-secondary"><b>${
          type1.names[5].name
        }</b>${getIcon(type1.name)}</td><td class="border border-secondary">${
    type2 != ""
      ? `<b>${type2.names[5].name + "</b>" + getIcon(type2.name)}`
      : ""
  }</td></tr>
        <tr><td class="border border-secondary" colspan="2"><b>RELACIÓN DE DAÑO</b></td></tr>
        <tr><td class="border border-secondary"><img src="/img/x4.png" /></td><td class="border border-secondary">${
          texto["4"]
        }</td></tr>        
        <tr><td class="border border-secondary"><img src="/img/x2.png" /></td><td class="border border-secondary">${
          texto["2"]
        }</td></tr>        
        <tr><td class="border border-secondary"><img src="/img/x1.png" /></td><td class="border border-secondary">${
          texto["1"]
        }</td></tr>        
        <tr><td class="border border-secondary"><img src="/img/medio.png" /></td><td class="border border-secondary">${
          texto["1/2"]
        }</td></tr>        
        <tr><td class="border border-secondary"><img src="/img/cuarto.png" /></td><td class="border border-secondary">${
          texto["1/4"]
        }</td></tr>
        <tr><td class="border border-secondary"><img src="/img/nulo.png" /></td><td class="border border-secondary">${
          texto["0"]
        }</td></tr>        
        <tr><td class="border border-secondary" colspan="2"><b>STATS BASE:</b> ${
          base_stats >= 500 ? "<b>" + base_stats + "</b>" : base_stats
        }<br><img id="grafico" src="${url}" /></td></tr>
        </table>
        `);
});
...
global.cargarEvoluciones = async function (especie, name) {
  function ordenAlfabetico(evolucionstring) {
    if (evolucionstring != "") {
      ...
      evolucionstring = ` >${evolucionstring.substring(
        5,
        evolucionstring.length
      )}`;
    }
  ...
  if (pokemonSpecies.evolution_chain && pokemonSpecies.evolution_chain.url) {
    var pokemonEvolutionChain = await P.resource(
